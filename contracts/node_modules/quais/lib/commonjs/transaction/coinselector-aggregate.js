...
class AggregateCoinSelector extends abstract_coinselector_js_1.AbstractCoinSelector {
    ...
    performSelection(config) {
        const { fee = BigInt(0), maxDenominationAggregate = 6, // Default to denomination[6] (1 Qi)
        maxDenominationOutput = utxo_js_1.denominations.length - 1, // Default to the last denomination
         } = config;
        this.validateUTXOs();
        // totalInputValue is the sum of the denominations of the eligible UTXOsregardless of maxDenomination
        this.totalInputValue = this.availableUTXOs.reduce((sum, utxo) => sum + BigInt(utxo_js_1.denominations[utxo.denomination]), BigInt(0));
        // get the UTXOs that are below or equal to maxDenomination
        const smallDenominationsUTXOs = this.availableUTXOs.filter((utxo) => utxo.denomination <= maxDenominationAggregate);
        ...
        const bigDenominationUTXOs = this.availableUTXOs.filter((utxo) => utxo.denomination > maxDenominationAggregate);
        ...
        if (fee > BigInt(0)) {
            ...
            const updatedValueToAggregate = inputsToAggregate.reduce((sum, utxo) => sum + BigInt(utxo_js_1.denominations[utxo.denomination]), BigInt(0));
        ...
        if (this.spendOutputs.length >= inputsToAggregate.length) {
            console.warn(`Aggregation would not reduce number of UTXOs, ${this.spendOutputs.length} outputs vs ${inputsToAggregate.length} inputs`);
        }
        this.changeOutputs = [];
        return {
     ...
     */
    createOptimalDenominations(value, maxDenominationIndex) {
        if (maxDenominationIndex < 0 || maxDenominationIndex >= utxo_js_1.denominations.length) {
    ...
    getInputsForFee(inputsToAggregate, availableUTXOs, fee) {
        ...
        while (currentInputsToAggregate.length > 0) {
            ...
            const eligiblefeeInputs = availableUTXOs.filter((utxo) => !currentInputsToAggregate.some((input) => input.txhash === utxo.txhash && input.index === utxo.index));
            // If no eligible fee inputs available, we need to remove inputs from aggregation
            if (eligiblefeeInputs.length === 0) {
                console.warn(`No UTXOs available for fee coverage, removing minimum UTXOs from aggregation to cover fee`);
                ...
                for (const utxo of sortedInputsToAggregate) {
                    feeSum += BigInt(utxo_js_1.denominations[utxo.denomination]);
                    utxosToRemove++;
                    if (feeSum >= fee) {
                        break;
                    }
        ...
        throw new Error(`Unable to find inputs to cover fee, wanted ${fee} but got 0 after removing all inputs from aggregation`);
    }
    ...
    getInputsToAggregate(smallDenominationsUTXOs, valueToAggregate) {
        ...
        throw new Error(`Unable to find inputs to aggregate, wanted ${valueToAggregate} but got ${inputsToAggregate.reduce((sum, utxo) => sum + BigInt(utxo_js_1.denominations[utxo.denomination]), BigInt(0))}`);
    }
}
