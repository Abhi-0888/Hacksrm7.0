...
let _cachedReactNativeCrypto = null;
let _cachedReactNativeQuickCrypto = null;
function getReactNativeFastCrypto() {
...
function getReactNativeQuickCrypto() {
...
function ser_I_ReactNative(index, chainCode, publicKey, privateKey, quickCrypto) {
    const data = new Uint8Array(37);
    if (index & HardenedBit) {
        (0, index_js_5.assert)(privateKey != null, 'cannot derive child of neutered node', 'UNSUPPORTED_OPERATION', {
            operation: 'deriveChild',
    ...
    const hmac = quickCrypto.createHmac('sha512', (0, index_js_5.getBytes)(chainCode));
    hmac.update(data);
...
function calculateFingerprintNative(publicKey, quickCrypto) {
    ...
    return (0, index_js_5.hexlify)(ripemdHash.subarray(0, 4));
}
...
class HDNodeWallet extends base_wallet_js_1.BaseWallet {
     ...
     */
    constructor(guard, signingKey, parentFingerprint, chainCode, path, index, depth, mnemonic, provider, childFingerprint, address) {
        ...
        const fingerprint = childFingerprint || (0, index_js_5.dataSlice)((0, index_js_1.ripemd160)((0, index_js_1.sha256)(this.publicKey)), 0, 4);
     ...
     */
    async deriveChildReactNative(_index) {
        const index = (0, index_js_5.getNumber)(_index, 'index');
        (0, index_js_5.assertArgument)(index <= 0xffffffff, 'invalid index', 'index', index);
        ...
        const childPriv = await fastCrypto.secp256k1.privateKeyTweakAdd((0, index_js_5.getBytes)(this.privateKey), // 32-byte parent key
        IL); // Buffer(32) → always valid key
        ...
        const compressedPub = (0, index_js_5.hexlify)(childPubCompressed);
        /* 4. Wrap in a *light* SigningKey without recomputing pubkey */
        const ki = new index_js_1.SigningKey((0, index_js_5.hexlify)(childPriv), compressedPub);
        ...
        const newWallet = new HDNodeWallet(_guard, ki, this.fingerprint, (0, index_js_5.hexlify)(IR), path, index, this.depth + 1, this.mnemonic, this.provider, fingerprint, address);
        return newWallet;
    }
    /**
     ...
     */
    async addressFromUncompressed(uncompressed, quickCrypto) {
        /* ----------------------------------------------------------- *
         * 1. keccak256( X || Y ) — skip the 0x04 prefix *
         * ----------------------------------------------------------- */
        const hashBytes = quickCrypto.keccak256(uncompressed.subarray(1)); // 32‑byte hash
        /* last 20 bytes are the raw address */
        const addrBytes = hashBytes.subarray(12); // Uint8Array(20)
        const addrHex = buffer_1.Buffer.from(addrBytes).toString('hex'); // 40 lowercase nibbles
        /* ----------------------------------------------------------- *
         * 2. keccak256( asciiLower(addr) ) — EIP-55 checksum *
         * ----------------------------------------------------------- */
        const chkBytes = quickCrypto.keccak256(buffer_1.Buffer.from(addrHex, 'utf8')); // ascii string → bytes
        const chkHex = buffer_1.Buffer.from(chkBytes).toString('hex'); // 64‑char hex string
        /* ----------------------------------------------------------- *
     ...
     */
    static async fromSeedReactNative(seed) {
        const quickCrypto = getReactNativeQuickCrypto();
        const seedBytes = (0, index_js_5.getBytes)(seed, 'seed');
        (0, index_js_5.assertArgument)(seedBytes.length >= 16 && seedBytes.length <= 64, 'invalid seed', 'seed', '[REDACTED]');
        ...
        const IR = I.slice(32);
        const signingKey = new index_js_1.SigningKey((0, index_js_5.hexlify)(IL));
        // return a depth‑0 node (path "m")
        // For depth 0 pass "0x00000000" as parentFingerprint
        return new HDNodeWallet(_guard, signingKey, '0x00000000', (0, index_js_5.hexlify)(IR), 'm', 0, 0, null, null);
    }
    /**
     * Native phrase -> seed -> derive path
     */
    static async fromMnemonicReactNative(mnemonic, path) {
        const fastCrypto = getReactNativeFastCrypto();
        const data = (0, index_js_4.toUtf8Bytes)(mnemonic.phrase, 'NFKD');
        const salt = (0, index_js_4.toUtf8Bytes)('mnemonic' + mnemonic.password, 'NFKD');
     ...
     */
    connectMnemonic(mnemonic) {
        return new HDNodeWallet(_guard, this.signingKey, this.parentFingerprint, this.chainCode, this.path, this.index, this.depth, mnemonic, this.provider, this.fingerprint, this.address);
    }
    /**
