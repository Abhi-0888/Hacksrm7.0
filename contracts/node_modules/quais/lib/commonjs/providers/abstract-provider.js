...
exports.AbstractProvider = exports.DefaultShardPorts = exports.UnmanagedSubscriber = void 0;
/**
...
class AbstractProvider {
    ...
    async initialize(urls) {
        ...
        try {
            ...
            const shardPortsArray = this.#options.shardPorts
                ? Array.isArray(this.#options.shardPorts)
                    ? this.#options.shardPorts
                    : [this.#options.shardPorts]
                : undefined;
            const shardPathsArray = this.#options.shardPaths
                ? Array.isArray(this.#options.shardPaths)
                    ? this.#options.shardPaths
                    : [this.#options.shardPaths]
                : undefined;
            const shardsArray = this.#options.shards
            ...
            if (urls instanceof index_js_3.FetchRequest) {
                ...
                if (shardsArray) {
                    ...
                    shardsArray[0].forEach((shard) => {
                        const shardNickname = (0, shards_js_2.fromShard)(shard, 'nickname');
                        const port = shardPorts && shardNickname in shardPorts
                            ? shardPorts?.[shardNickname]
                            : exports.DefaultShardPorts[shardNickname];
                        ...
                        const fetchRequest = new index_js_3.FetchRequest(this.constructUrl(urls.url, shardSuffix));
                        if (isFirst) {
                            this.#connect.push(fetchRequest);
                            isFirst = false;
                        }
                        this._urlMap.set(shard, fetchRequest);
                    });
                }
                else {
                    if (!this.#options.usePathing) {
                        // When usePathing is false, use single URL for Cyprus1 (default active shard)
                        this._urlMap.set(index_js_2.Shard.Cyprus1, urls);
                        ...
                        shards.forEach((shard) => {
                            ...
                            const shardNickname = (0, shards_js_2.fromShard)(shardEnum, 'nickname');
                            const port = shardPorts && shardNickname in shardPorts
                                ? shardPorts?.[shardNickname]
                                : exports.DefaultShardPorts[shardNickname];
                            const path = shardPaths && shardNickname in shardPaths
                                ...
                                : `/${shardNickname}`;
                            const shardSuffix = this.#options.usePathing ? `${path}` : `:${port}`;
                            this._urlMap.set(shardEnum, new index_js_3.FetchRequest(this.constructUrl(urls.url, shardSuffix)));
                        });
                    }
                }
                return;
                    ...
                    if (shardsArray) {
                        ...
                        shardsArray[urlIndex].forEach((shard) => {
                            ...
                            const path = shardPaths && shardNickname in shardPaths
                                ...
                                : `/${shardNickname}`;
                            const shardSuffix = this.#options.usePathing ? `${path}` : `:${port}`;
                            const fetchRequest = new index_js_3.FetchRequest(this.constructUrl(urls[urlIndex], shardSuffix));
                            if (isFirst) {
                                this.#connect.push(fetchRequest);
                                isFirst = false;
                            }
                        ...
                        if (!this.#options.usePathing) {
                            // When usePathing is false, use single URL for Cyprus1 (default active shard)
                            const connect = new index_js_3.FetchRequest(urls[urlIndex]);
                            this._urlMap.set(index_js_2.Shard.Cyprus1, connect);
                            ...
                            shards.forEach((shard) => {
                                ...
                                const path = shardPaths && shardNickname in shardPaths
                                    ...
                                    : `/${shardNickname}`;
                                const shardSuffix = this.#options.usePathing ? `${path}` : `:${port}`;
                                this._urlMap.set((0, index_js_2.toShard)(`0x${shard[0].toString(16)}${shard[1].toString(16)}`), new index_js_3.FetchRequest(this.constructUrl(urls[urlIndex], shardSuffix)));
                            });
                        }
                    }
                }
    ...
    async getFeeData(zone, txType = true) {
        const getFeeDataFunc = async () => {
            const { gasPrice } = await (0, index_js_3.resolveProperties)({
                gasPrice: (async () => {
                    try {
            ...
            return new provider_js_1.FeeData(gasPrice);
        };
    ...
    async calculateConversionAmount(fromOrArgs, to, value) {
        ...
        if (typeof fromOrArgs === 'string' && to && value !== undefined) {
            ...
            formattedArgs = {
                ...
                value: (0, index_js_3.toQuantity)(fromOrArgs.value),
            };
        }
        else {
        ...
        return (0, index_js_3.getBigInt)(result, '%response');
    }
