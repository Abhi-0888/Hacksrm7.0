...
import { AbstractHDWallet, _guard } from './abstract-hdwallet.js';
...
import { denominations, UTXO } from '../transaction/utxo.js';
...
class QiHDWallet extends AbstractHDWallet {
     ...
     * @readonly
     */
    static bip47derivationPath = "m/47'/969'";
    /**
     ...
     */
    async getNextAddress(account, zone, useReactNative = false) {
        if (useReactNative) {
     ...
     */
    async signTransaction(tx, onProgress) {
    ...
    outpointsForAddress(address) {
        return this.getOutpoints(getZoneForAddress(address) || Zone.Cyprus1).filter((outpoint) => outpoint.address === address);
    }
    /**
     ...
     */
    async aggregate(zone, options = {}, maxDenominationAggregate = 6, maxDenominationOutput = denominations.length - 1, onProgress) {
        ...
        onProgress?.(60, 'Preparing transaction', `Creating transaction with ${selection.inputs.length} inputs and ${selection.spendOutputs.length + selection.changeOutputs.length} outputs`);
        ...
        onProgress?.(70, 'Signing transaction', `Signing transaction with ${tx.txInputs.length} inputs using ${tx.txInputs.length === 1 ? 'Schnorr' : 'MuSig'}`);
     ...
     */
    calculateTransactionFee(selection) {
     ...
     */
    calculateActualTransactionFee(tx) {
        const inputDenominations = [];
        const outputDenominations = [];
        // Calculate total input value
        const totalInputValue = tx.txInputs.reduce((sum, input) => {
            // Get the denomination from the UTXO
            const utxo = this.outpointsForAddress(computeAddress(input.pubkey)).find((outpoint) => outpoint.outpoint.txhash === input.txhash && outpoint.outpoint.index === input.index);
     ...
     */
    validateTransactionMatchesSelection(tx, selection) {
        // Validate input count
        if (tx.txInputs.length !== selection.inputs.length) {
            throw new Error(`Transaction input count mismatch: expected ${selection.inputs.length}, got ${tx.txInputs.length}`);
        }
        ...
        if (tx.txOutputs.length !== expectedOutputCount) {
            throw new Error(`Transaction output count mismatch: expected ${expectedOutputCount}, got ${tx.txOutputs.length}`);
        }
        ...
        for (let i = 0; i < tx.txInputs.length; i++) {
            ...
            if (txInput.txhash !== selectionInput.txhash || txInput.index !== selectionInput.index) {
                throw new Error(`Transaction input mismatch at index ${i}: expected ${selectionInput.txhash}:${selectionInput.index}, got ${txInput.txhash}:${txInput.index}`);
            }
        }
        ...
        if (expectedFee !== actualFee) {
            throw new Error(`Transaction fee mismatch: coin selector expected ${Number(expectedFee) / 1000} Qi, but actual transaction has ${Number(actualFee) / 1000} Qi fee`);
        }
    }
    /**
     ...
     */
    async getChangeAddressesForOutputs(count, zone, account = 0) {
     ...
     */
    getUnusedBIP44Addresses(amount, account, path, zone) {
        ...
        const newAddresses = Array.from({ length: remainingAddressesNeeded }, () => wallet.deriveNewAddress(zone, account));
     ...
     */
    async createMuSigSignature(tx, hash, onProgress) {
    ...
    async sync(zone, account = 0, onOutpointsCreated, onOutpointsDeleted) {
        ...
        const paymentChannelSyncs = Array.from(this.paymentChannels.values()).map((pc) => pc.selfWallet.sync(zone, account, onOutpointsCreated, onOutpointsDeleted));
    ...
    serialize() {
        ...
        const paymentChannelSelfAddresses = Array.from(this.paymentChannels.values()).flatMap((pc) => pc.selfWallet.exportAllAddresses());
        ...
        const senderPaymentCodeInfo = Object.fromEntries(Array.from(this.paymentChannels.entries()).map(([paymentCode, channel]) => [
            paymentCode,
