...
export class AggregateCoinSelector extends AbstractCoinSelector {
    ...
    performSelection(config) {
        ...
        this.totalInputValue = this.availableUTXOs.reduce((sum, utxo) => sum + BigInt(denominations[utxo.denomination]), BigInt(0));
        ...
        if (fee > BigInt(0)) {
            ...
            const updatedValueToAggregate = inputsToAggregate.reduce((sum, utxo) => sum + BigInt(denominations[utxo.denomination]), BigInt(0));
    ...
    getInputsToAggregate(smallDenominationsUTXOs, valueToAggregate) {
        ...
        throw new Error(`Unable to find inputs to aggregate, wanted ${valueToAggregate} but got ${inputsToAggregate.reduce((sum, utxo) => sum + BigInt(denominations[utxo.denomination]), BigInt(0))}`);
    }
}
