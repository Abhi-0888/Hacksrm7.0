...
// ── Config ──
let ACTIVE_CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EVENT_DAO_ADDRESS || '';
...
// Lazy load quais to prevent Webpack SES lockdown crash during Next.js SSR
let quaisModule: any = null;
async function getQuais() {
  if (!quaisModule) {
    quaisModule = await import('quais');
  }
  return quaisModule.quais;
}

// ── Provider via Pelagus ──
async function getBrowserProvider(): Promise<any> {
  const quais = await getQuais();
